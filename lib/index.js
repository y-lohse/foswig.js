 ...
 * @param {function} PRNG A pseudo-random number generator function
 ...
 */
module.exports.prototype.generateWord = function(PRNG, minLength,maxLength,allowDuplicates,maxAttempts) {
	if (typeof(PRNG)==='undefined') PRNG = Math.random;
	...
	{
		repeat = false;
		var nextNodeIndex = Math.floor(PRNG() * this.start.neighbors.length);
		...
		while (currentNode && (maxLength < 0 || word.length<=maxLength)) {
			...
			nextNodeIndex = Math.floor(PRNG() * currentNode.neighbors.length);
